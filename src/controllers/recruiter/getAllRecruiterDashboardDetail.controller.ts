...
import { getAllRecruiterDashboardDetailService } from "../../services/request/getAllRecruiterDashboardDetail.service";
...
export const getAllRecruiterDashboardDetailController = async (
  req: Request,
  res: Response
) => {
  const { uuid } = req.user;

  const validateUser = (await retrieveIfUserExists(uuid)) as RecruiterEntity;

  getAllRecruiterDashboardDetailService(validateUser, {
    relations: {
      requests: {
        candidate: {
    ...
    .then((data) => {
      ...
      const dashboardResume = {
        totalActiveVacancies: activeVacancies?.length || 0,
        totalRecentCandidates: recentCandidates?.length || 0,
...
export const filterRecruitmentActivityByMonth = (
  ...
  return getRecruitmentActivityCandidateResume(dataFilteredByCurrentMonthAndYear);
};
...
export const getRecruitmentActivityCandidateResume = (data: JobPositionEntity[]) => {
return {
...
export const getTotalRequestByStatus = (
  ...
  return (
    data
      ?.filter((dashboard) =>
        dashboard?.requests?.filter((r) => r?.status === status)
      )
      ?.map((dashboard) => dashboard?.requests)?.length || 0
  );
};
